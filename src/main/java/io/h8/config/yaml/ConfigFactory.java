...
/**
 * Drop-in replacement for {@link com.typesafe.config.ConfigFactory} that automatically
 * attaches {@link YamlConfigIncluder#DEFAULT} to all parse and load operations.
 *
 * <p>Replacing {@code import com.typesafe.config.ConfigFactory} with
 * {@code import io.h8.config.yaml.ConfigFactory} is sufficient — no other changes needed.
 *
...
public final class ConfigFactory {
    ...
    /**
     * Equivalent to {@link com.typesafe.config.ConfigFactory#load(ClassLoader, ConfigParseOptions, ConfigResolveOptions)}
     * with {@link YamlConfigIncluder#DEFAULT} prepended.
     *
     * @param loader         class loader used to find resources
    ...
    /**
     * Equivalent to {@link com.typesafe.config.ConfigFactory#load(ConfigParseOptions, ConfigResolveOptions)}
     * with {@link YamlConfigIncluder#DEFAULT} prepended to {@code parseOptions}.
     *
     * @param parseOptions   parse options
     * @param resolveOptions substitution resolution options
    ...
    public static Config load(ClassLoader loader, ConfigParseOptions parseOptions, ConfigResolveOptions resolveOptions) {
        return com.typesafe.config.ConfigFactory.load(loader, withIncluder(parseOptions), resolveOptions);
    }

    /**
     * Equivalent to {@link com.typesafe.config.ConfigFactory#load(String, ConfigParseOptions, ConfigResolveOptions)}
     * with {@link YamlConfigIncluder#DEFAULT} prepended.
     *
    ...
    /**
     * Equivalent to {@link com.typesafe.config.ConfigFactory#load(ClassLoader, String, ConfigParseOptions, ConfigResolveOptions)}
     * with {@link YamlConfigIncluder#DEFAULT} prepended.
     *
     * @param loader           class loader used to find resources
     * @param resourceBasename resource basename (e.g. {@code application})
    ...
    public static Config load(String resourceBasename, ConfigParseOptions parseOptions, ConfigResolveOptions resolveOptions) {
        return com.typesafe.config.ConfigFactory.load(resourceBasename, withIncluder(parseOptions), resolveOptions);
    }
    ...
    public static Config load(ClassLoader loader, String resourceBasename, ConfigParseOptions parseOptions, ConfigResolveOptions resolveOptions) {
        return com.typesafe.config.ConfigFactory.load(loader, resourceBasename, withIncluder(parseOptions), resolveOptions);
    }

    /**
     * Delegates directly to {@link com.typesafe.config.ConfigFactory#load(Config)}.
     * No includer is injected — the config is already parsed.
     *
    ...
    /**
     * Delegates directly to {@link com.typesafe.config.ConfigFactory#load(Config, ConfigResolveOptions)}.
     * No includer is injected — the config is already parsed.
     *
     * @param config         already-parsed config to resolve
     * @param resolveOptions substitution resolution options
    ...
    /**
     * Delegates directly to {@link com.typesafe.config.ConfigFactory#load(ClassLoader, Config, ConfigResolveOptions)}.
     * No includer is injected — the config is already parsed.
     *
     * @param loader         class loader used for resolution
     * @param config         already-parsed config to resolve
    ...
    /**
     * Equivalent to {@link com.typesafe.config.ConfigFactory#defaultApplication(ConfigParseOptions)}
     * with {@link YamlConfigIncluder#DEFAULT} prepended.
     *
    ...
    /**
     * Equivalent to {@link com.typesafe.config.ConfigFactory#defaultApplication(ConfigParseOptions)}
     * with {@link YamlConfigIncluder#DEFAULT} prepended and {@code loader} set via
     * {@link ConfigParseOptions#setClassLoader(ClassLoader)}.
     *
    ...
    /**
     * Returns the reference config assembled from all {@code reference.conf} resources on the classpath.
     *
    ...
    /**
     * Returns the reference config assembled from all {@code reference.conf} resources found by {@code loader}.
     *
    ...
    /**
     * Returns the unresolved reference config assembled from all {@code reference.conf} resources on the classpath.
     *
    ...
    /**
     * Returns the unresolved reference config assembled from all {@code reference.conf} resources found by {@code loader}.
     *
     * @param loader class loader used to find {@code reference.conf} resources
     * @return the unresolved reference config
    ...
    /**
     * Returns config from system-property overrides and other highest-priority sources,
     * using {@code loader} for any classpath lookups.
     *
    ...
    /**
     * Equivalent to {@link com.typesafe.config.ConfigFactory#parseProperties(Properties, ConfigParseOptions)}
    ...
    /**
     * Equivalent to {@link com.typesafe.config.ConfigFactory#parseReader(Reader, ConfigParseOptions)}
     * with {@link YamlConfigIncluder#DEFAULT} prepended to {@code options}.
     *
    ...
    /**
     * Equivalent to {@link com.typesafe.config.ConfigFactory#parseFileAnySyntax(File, ConfigParseOptions)}
     * with {@link YamlConfigIncluder#DEFAULT} prepended to {@code options}.
     *
    ...
    /**
     * Equivalent to {@link com.typesafe.config.ConfigFactory#parseResources(Class, String, ConfigParseOptions)}
     * with {@link YamlConfigIncluder#DEFAULT} prepended to {@code options}.
     *
    ...
    public static Config parseResources(Class<?> klass, String resource, ConfigParseOptions options) {
        return com.typesafe.config.ConfigFactory.parseResources(klass, resource, withIncluder(options));
    }
    ...
    /**
     * Equivalent to {@link com.typesafe.config.ConfigFactory#parseResourcesAnySyntax(Class, String, ConfigParseOptions)}
     * with {@link YamlConfigIncluder#DEFAULT} prepended to {@code options}.
     *
     * @param klass    class used for classpath-relative resource lookup
     ...
     * @return parsed {@link Config}
     */
    public static Config parseResourcesAnySyntax(Class<?> klass, String resource, ConfigParseOptions options) {
        return com.typesafe.config.ConfigFactory.parseResourcesAnySyntax(klass, resource, withIncluder(options));
    }
    ...
    /**
     * Equivalent to {@link com.typesafe.config.ConfigFactory#parseResources(ClassLoader, String, ConfigParseOptions)}
     * with {@link YamlConfigIncluder#DEFAULT} prepended to {@code options}.
     *
    ...
    public static Config parseResources(ClassLoader loader, String resource, ConfigParseOptions options) {
        return com.typesafe.config.ConfigFactory.parseResources(loader, resource, withIncluder(options));
    }
    ...
    /**
     * Equivalent to {@link com.typesafe.config.ConfigFactory#parseResourcesAnySyntax(ClassLoader, String, ConfigParseOptions)}
     * with {@link YamlConfigIncluder#DEFAULT} prepended to {@code options}.
     *
     * @param loader   class loader used to find the resource
     * @param resource classpath resource path (extension is inferred)
     ...
     * @return parsed {@link Config}
     */
    public static Config parseResourcesAnySyntax(ClassLoader loader, String resource, ConfigParseOptions options) {
        return com.typesafe.config.ConfigFactory.parseResourcesAnySyntax(loader, resource, withIncluder(options));
    }
    ...
    /**
     * Equivalent to {@link com.typesafe.config.ConfigFactory#parseResources(String, ConfigParseOptions)}
     * with {@link YamlConfigIncluder#DEFAULT} prepended to {@code options}.
     *
     * @param resource classpath resource path
     * @param options  parse options
    ...
    /**
     * Equivalent to {@link com.typesafe.config.ConfigFactory#parseResourcesAnySyntax(String, ConfigParseOptions)}
     * with {@link YamlConfigIncluder#DEFAULT} prepended to {@code options}.
     *
     * @param resource classpath resource path (extension is inferred)
     * @param options  parse options
    ...
    public static Config parseResourcesAnySyntax(String resource, ConfigParseOptions options) {
        return com.typesafe.config.ConfigFactory.parseResourcesAnySyntax(resource, withIncluder(options));
    }
    ...
    /**
     * Equivalent to {@link com.typesafe.config.ConfigFactory#parseApplicationReplacement(ConfigParseOptions)}
     * with {@link YamlConfigIncluder#DEFAULT} prepended.
     *
    ...
    /**
     * Equivalent to {@link com.typesafe.config.ConfigFactory#parseString(String, ConfigParseOptions)}
     * with {@link YamlConfigIncluder#DEFAULT} prepended to {@code options}.
     *
