...
public final class YamlConfigFactory {
   ...
   * @return one {@link ConfigValue} per document, in stream order; empty if the string contains no
   *     documents
  ...
  public List<ConfigValue> parseResources(Class<?> klass, String resource) {
    ...
    if (url == null)
      throw new ConfigException.IO(
          ConfigOriginFactory.newSimple(resource), "resource not found on classpath: " + resource);
    return parseURL(url);
  }
  ...
  private List<ConfigValue> parseAll(StreamReader stream, String originDesc) {
    ...
    try {
      ...
      return StreamSupport.stream(
              Spliterators.spliteratorUnknownSize(composer, Spliterator.ORDERED), false)
          .map(converter)
